...
class FlangObject:
    ...
    def root_component(self) -> FlangComponent:
        ...
        assert isinstance(
            root, FlangComponent
        ), "The root construct should be a component!"
...
class FlangMatchObject:
    ...
    def to_dict(self) -> RecursiveDict:
        ...
        return {key: value.to_dict() for key, value in self.spec_or_matched.items()}
    ...
    @classmethod
    def empty_match(cls, start_position: int) -> FlangMatchObject:
        return cls(position=Postition(start_position, start_position), spec_or_matched="")
