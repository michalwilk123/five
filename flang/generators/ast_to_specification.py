...
def generate_specification_for_cardinality(
    ...
    ctr = Counter(
        [template.full_search(item.template_id).get_id() for item in branch.children]
    )
    children_card_dict = {}
...
def generate_specification(
    template_tree: TemplateTree, user_ast: FlangAST
) -> Specification:
    if isinstance(user_ast, FlangRoot):
        fake_root = TemplateRoot()
        fake_root.add_node(template_tree)
        template = fake_root
    else:
        template = template_tree.full_search(user_ast.template_id)
    ...
    if isinstance(user_ast, FlangLeaf):
        specification |= generate_specification_for_leaf(template, user_ast)
    elif isinstance(user_ast, FlangBranch):
        specification |= generate_specification_for_branch(template, user_ast)
